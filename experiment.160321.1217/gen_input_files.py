...
def main():
    ...
    # Generate 10 files for each capacity in 3 separate folders
    # corresponding to each processor core available for use.
    ...
    for capacity in capacities:
        for fnum in range(1,4):
            ...
            create_output_file(pallets, total, capacity, fnum, dir="1")
        for fnum in range(4,7):
            total, pallets = fill_with_equal_probability(capacity)
            create_output_file(pallets, total, capacity, fnum, dir="2")
        for fnum in range(7,11):
            total, pallets = fill_with_equal_probability(capacity)
            create_output_file(pallets, total, capacity, fnum, dir="3")

        dirname = "c" + str(capacity)
        for fnum in range(11, num_of_files_per_capacity + 1):
            total, pallets = fill_with_equal_probability(capacity)
